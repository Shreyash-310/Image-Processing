...
for o in ImageFiles:
    g = "python predict.py -c config.json -w trained_wts.h5 -i " + o
    os.system(g)
    os.remove(o)
    files = [i for i in os.listdir(path) if
             os.path.isfile(os.path.join(path, i)) and 'Part_' in i]
    for j in files:
...
f.write(
    "Trip Time in Seconds,ImageFile,PotholeValue,DataDriven, "
    "Longitude, Latitude\n")
for k in ImageDetected:
    ...
    if len(a) > 0:
        c_array = np.array(c)
        f.write(str(df_File.iloc[
                        int(np.average(c_array) + int(t[0])), 73]) + ',' + str(
            k) + ',' + str(a) + ',' + 'Confirmend With Data,' + str(
            df_File.iloc[int(np.average(c_array) + int(t[0])), 2]) + ',' + str(
            df_File.iloc[int(np.average(c_array) + int(t[0])), 3]) + '\n')

        MapPloting.PlotingMap(
            float(df_File.iloc[int(np.average(c_array) + int(t[0])), 2]),
            float(df_File.iloc[int(np.average(c_array) + int(t[0])), 3]),
            str(t[0]))
